...
class Cst {
	...
	public function createConnection() {
		...
		} else if (in_array($this->connectionType, array('Cloudfiles', 'clodo'))) {
			require_once CST_DIR.'/lib/api/cloudfiles.php';
			try {
				if($this->connectionType == 'clodo')
				{
					$region = 'https://api.clodo.ru';
					$_t = 'clodo';
				}
				else
				{
				...
				$cfAuth = new CF_Authentication(get_option('cst-' . $_t . '-username'), get_option('cst-' . $_t . '-api'), NULL, $region);
				$cfAuth->authenticate();
				$this->cdnConnection = new CF_Connection($cfAuth);
				$this->cdnConnection = $this->cdnConnection->create_container(get_option('cst-' . $_t . '-container'));
			} catch (Exception $e) {
	...
	public function syncFiles() {
		...
		if ($this->connectionType == 'Origin') {
			...
			$i = 1;
			//echo "SELECT * FROM `".CST_TABLE_FILES."` WHERE `synced` = '0'" . "<br/>";
